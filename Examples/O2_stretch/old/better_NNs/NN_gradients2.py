...
# train with 10 datapoints. validate with 10. test with 90
...
#test_x = data.iloc[1::4, :-2].values +  data.iloc[2::4, :-2].values + data.iloc[3::4, :-2].values
#test_y = data.iloc[1::4, -2:].values +  data.iloc[2::4, -2:].values + data.iloc[3::4, -2:].values
#print(len(test_x))

# validation set, take the 4th, 14th, 24th, etc
valid_x = data.iloc[4::10, :-2].values
valid_y = data.iloc[4::10, -2:].values

# training set, take the 0th, 10th, 20th, etc
train_x = data.iloc[::10, :-2].values
train_y = data.iloc[::10, -2:].values

# test set, take the 1st, 2nd, 3rd, 5th, 6th, 7th, etc
test_x = [i for i in data.iloc[:, :-2].values if (i not in valid_x and i not in train_x) ]
test_y = [i for i in data.iloc[:, -2:].values if (i not in valid_y and i not in train_y) ]
