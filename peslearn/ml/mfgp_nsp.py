...
class MFGP_NSP(GaussianProcess):
    def __init__(self, dataset_path, lf_dataset_path, input_obj, input_obj_l, molecule_type=None, molecule=None, train_path=None, test_path=None, train_path_low=None, test_path_low=None, epochs=(100,100)):
    ...
    def build_model(self, params, nrestarts=10, maxiter=1000, seed=0):
        ...
        mll_l = gpytorch.mlls.ExactMarginalLogLikelihood(self.likelihood_l, self.model_l)
        ...
        for i in range(self.epochs_l):
            ...
            #opt_hyp.zero_grad()
            opt_l.zero_grad()
            out = self.model_l(self.Xtr_l)
            loss = -mll_l(out, torch.squeeze(self.ytr_l))
            loss.backward()
            opt_l.step()
            #opt_ngd.step()
            #opt_hyp.step()
    ...
    def predict(self, hf_model, lf_model, x_in):
        ...
        #with torch.no_grad(), gpytorch.settings.fast_pred_var():
        #    lf_pred = lf_model(x_in).mean.unsqueeze(-1)
        #    xx = torch.hstack((x_in.squeeze(0), lf_pred.squeeze(0)))
        #    prediction = hf_model(xx).mean.unsqueeze(1)
        #return prediction 
    ...
    def vet_model(self, model):
        ...
        #pred_test = self.predict(model, self.model_l, self.Xtest)
        #pred_full = self.predict(model, self.model_l, self.X)
        #error_test = self.compute_error(self.ytest.squeeze(0), pred_test, self.yscaler)
        #error_full, median_error, max_errors = self.compute_error(self.y.squeeze(0), pred_full, yscaler=self.yscaler, max_errors=5)
        #print("Test Dataset {}".format(round(hartree2cm * error_test,2)), end='  ')
        #print("Full Dataset {}".format(round(hartree2cm * error_full,2)), end='     ')
